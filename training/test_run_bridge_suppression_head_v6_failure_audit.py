...
class TestRunBridgeSuppressionHeadV6FailureAudit(unittest.TestCase):
    ...
    def test_runner_is_read_only_and_never_uses_test_holdout_or_threshold_sweep(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            with mock.patch.object(runner.split_runner, "_prepare_manifest", return_value={"device": torch.device("cpu"), "manifest": {"contract": {"train_sample_ids": ["train_1"], "val_sample_ids": ["val_1"]}}}), \
                 mock.patch.object(runner.v6, "build_v6_model_with_fresh_bridge_head", return_value=(mock.Mock(), {}, {})), \
                 mock.patch.object(runner.bridge, "mine_bridge_records_for_split", mine_mock), \
                 mock.patch.object(runner.v6, "build_v6_cached_records", side_effect=lambda rows: rows), \
                 mock.patch.object(runner.audit, "AUTHORITATIVE_V6_CHECKPOINT", checkpoint_path), \
                 mock.patch.object(runner.v6, "DEFAULT_RUN_DIR", run_dir), \
                 mock.patch.object(runner.audit, "load_v6_model_and_verify_invariants", return_value=(mock.Mock(), {"semantic_checkpoint": {"checkpoint_sha256": "semantic"}, "semantic_parameter_max_delta": 0.0, "semantic_bn_state_max_delta": 0.0})), \
                 mock.patch.object(runner.v6, "evaluate_open_on_cached_records", side_effect=[train_eval, val_eval]), \
                 mock.patch.object(runner.audit, "_bridge_probs_for_cached_records", side_effect=[np.zeros((1, 1, 2, 2), dtype=np.float32), np.zeros((1, 1, 2, 2), dtype=np.float32), np.zeros((1, 1, 2, 2), dtype=np.float32), np.zeros((1, 1, 2, 2), dtype=np.float32)]), \
                 mock.patch.object(runner.audit, "load_historical_v2_model", return_value=(mock.Mock(), {"checkpoint_sha256": "hist_v2"})), \
                 mock.patch.object(runner.audit, "summarize_score_partition", return_value=score_summary), \
                 mock.patch.object(runner.audit, "damage_decomposition", return_value={"aggregate": {"train_positive_target_samples": {"inside_gt_removed_pixels": 1, "outside_gt_removed_pixels": 0, "boundary_distance_bins": {"0_2": 1, "3_5": 0, "6_10": 0, "gt_10": 0}}}, "per_sample_rows": []}), \
                 mock.patch.object(runner.audit, "removal_component_morphology", return_value={"aggregate": {"total_component_count": 1}, "component_area_distribution": {}, "per_sample_rows": [], "per_component_rows": []}), \
                 mock.patch.object(runner.audit, "topology_failure_cases", return_value=[{"sample_id": "val_1", "failure_class": "A_leaflet_interior_cut"}]), \
                 mock.patch.object(runner.audit, "exact_target_oracle", side_effect=[exact_oracle, exact_oracle]), \
                 mock.patch.object(runner.audit, "pareto_frontier", return_value=pareto), \
                 mock.patch.object(runner.audit, "compare_historical_v2_vs_v6", return_value=compare_payload), \
                 mock.patch.object(runner.audit, "interpret_historical_v2_vs_v6", return_value={"main_improvement": "bridge recall with higher removal aggressiveness", "main_unchanged_failure": "inside-GT destructive negative removal"}), \
                 mock.patch.object(runner.audit, "patient_level_failure_concentration", return_value={"rows": [{"patient_id": "p2"}]}), \
                 mock.patch.object(runner.audit, "classify_dominant_failure", return_value={"dominant_failure_class": ["A_SCORE_SEPARABILITY_FAILURE"], "recommended_representation_family": "context-conditioned separation head"}):
