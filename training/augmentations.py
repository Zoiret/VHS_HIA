...
class TrainAugmentations:
    ...
    def __call__(self, image: np.ndarray, mask: np.ndarray, boundary: np.ndarray | None = None):
        ...
        if boundary is None:
            ...
            if deterministic_crop:
                image, mask, boundary = _center_crop(image, mask, self.input_h, self.input_w, boundary=boundary)
            else:
                image, mask, boundary = _random_crop(image, mask, self.input_h, self.input_w, boundary=boundary)
...
class ValAugmentations:
    ...
    def __call__(self, image: np.ndarray, mask: np.ndarray, boundary: np.ndarray | None = None):
        ...
        return _center_crop(image, mask, self.input_h, self.input_w, boundary=boundary)
